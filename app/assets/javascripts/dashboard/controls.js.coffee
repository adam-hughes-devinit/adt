...
App.render_url_from_html_state = () ->
	...
	console.log 'navigating to', x_axis, url_params
	Finch.navigate "/#{x_axis}", url_params
...
App.get_all_filters = () ->
	filter_types = App.config.data.columns.filter((c) -> c.interface_type == 'filter').map((c) -> c.name)
	filters = []
...
App.make_with_new_x_axis = (column) ->
