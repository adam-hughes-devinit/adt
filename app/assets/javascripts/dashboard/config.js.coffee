...
App.config = 
	...
	data:
		...
		columns: [
			...
			#	
			{
				# right now, aggregation is hard-coded to "amount" in chart.coffee
				name: "amount",
				values_function: (data) -> data.map((d) -> Math.round(parseFloat(d.usd_defl || 0)) ), #  d.commitment_usd_constant_sum 
			...
			{
				name: "recipient",
				values_function: (data) -> data.map((d) -> d.recipient_condensed),
				interface_type: "filter",
