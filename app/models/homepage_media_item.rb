class HomepageMediaItem < ActiveRecord::Base
  ...
  def youtube_embed(youtube_url,height,width,iframe_id)
    ...
    %Q{<iframe id="slide_#{ iframe_id }" title="YouTube video player" width="#{ width }" height="#{ height }" src="http://www.youtube.com/embed/#{ youtube_id }?enablejsapi=1" frameborder="0" allowfullscreen></iframe>}
  end
end
